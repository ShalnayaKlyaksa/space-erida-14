...
public sealed class CustomColorSelectorSliders : Control
{
    ...
    private static readonly HsvSliderStrategy HsvStrategy = new();
    private static readonly RgbSliderStrategy RgbStrategy = new();
    ...
    private bool _isAlphaVisible;

    private readonly ColorableSlider _topColorSlider;
    private readonly ColorableSlider _middleColorSlider;
    private readonly ColorableSlider _bottomColorSlider;
    private readonly Slider _alphaSlider;

    private readonly BoxContainer _alphaSliderBox = new();

    private readonly SpinBox _topInputBox;
    private readonly SpinBox _middleInputBox;
    private readonly SpinBox _bottomInputBox;
    private readonly SpinBox _alphaInputBox;

    private readonly Label _topSliderLabel = new();
    private readonly Label _middleSliderLabel = new();
    private readonly Label _bottomSliderLabel = new();
    private readonly Label _alphaSliderLabel = new();
    ...
    public CustomColorSelectorSliders() : this(ColorSelectorType.Rgb)
    {
    }

    public CustomColorSelectorSliders(ColorSelectorType type = ColorSelectorType.Rgb, string? label = null)
    {
        ...
        _topColorSlider.OnValueChanged += _ => OnSliderValueChanged(ColorSliderOrder.Top);
        _middleColorSlider.OnValueChanged += _ => OnSliderValueChanged(ColorSliderOrder.Middle);
        _bottomColorSlider.OnValueChanged += _ => OnSliderValueChanged(ColorSliderOrder.Bottom);
        _alphaSlider.OnValueChanged += _ => OnSliderValueChanged(ColorSliderOrder.Alpha);
        ...
        foreach (var selectorType in Enum.GetValues<ColorSelectorType>())
        {
            _typeSelector.AddItem(Loc.GetString($"color-selector-sliders-{selectorType.ToString().ToLower()}"));
            _types.Add(selectorType);
        }
    ...
    private ColorSliderStrategy GetStrategy()
    {
        ...
        {
            ColorSelectorType.Rgb => RgbStrategy,
            ColorSelectorType.Hsv => HsvStrategy,
